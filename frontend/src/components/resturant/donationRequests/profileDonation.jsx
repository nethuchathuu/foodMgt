...
import { X, MapPin, Phone, Mail, Building2, User, Clock, HeartHandshake } from 'lucide-react';

const ProfileDonation = ({ isOpen, onClose, requester }) => {
  if (!requester) return null;

  const isOrg = requester.type === "Organization";
          ...
          >
            ...
            <div className="px-8 pb-8 pt-0 relative">
              ...
              <div className="mb-6">
                ...
                {isOrg && requester.orgName && requester.orgName !== requester.name && (
                  <p className="text-gray-500 text-sm font-medium mt-1">{requester.orgName}</p>
                )}
                <div className="flex gap-4 mt-3 text-sm text-[#A7D63B] font-bold bg-[#A7D63B]/10 w-fit px-3 py-1.5 rounded-lg border border-[#A7D63B]/20">
                  <span className="flex items-center gap-1.5 ">
                    <HeartHandshake size={16} /> 42 Donations Received
                  </span>
                </div>
              </div>
              ...
              <div className="space-y-3 mb-8">
                <div className="flex items-center gap-4 p-3 bg-gray-50 rounded-xl border border-gray-100 hover:border-gray-200 transition-colors">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-semibold text-gray-700">{requester.phone}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-4 p-3 bg-gray-50 rounded-xl border border-gray-100 hover:border-gray-200 transition-colors">
                  ...
                  <div className="overflow-hidden">
                    ...
                    <p className="text-sm font-semibold text-gray-700 truncate">{requester.email}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-4 p-3 bg-gray-50 rounded-xl border border-gray-100 hover:border-gray-200 transition-colors">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-semibold text-gray-700 leading-tight">{requester.address}</p>
                  </div>
                </div>
              </div>
              ...
              <button className="w-full py-3 rounded-xl bg-[#1F5E2A] text-white font-bold hover:bg-opacity-90 transition-all shadow-md hover:shadow-lg active:scale-95 flex justify-center items-center gap-2">
                <Mail size={18} />
                Message {isOrg ? 'Organization' : 'User'}
              </button>
