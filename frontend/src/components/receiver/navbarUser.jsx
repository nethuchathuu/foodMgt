...
export default function NavbarUser() {
  ...
  const handleProfileClick = () => {
    ...
    try {
      ...
      if (role === 'requester_org') {
        navigate('/receiver/profileOrganization');
      } else {
        navigate('/receiver/profileIndividual');
      }
    } catch (e) {
    ...
    <header className="sticky top-0 z-30 w-full bg-white shadow-sm flex items-center justify-between px-6 py-4" style={{ backgroundColor: '#F8F8F6', borderBottom: '1px solid #D8C3A5' }}>
      ...
      <div className="flex items-center gap-4">
        ...
        <div 
          ...
          className="w-10 h-10 rounded-full bg-gray-200 flex flex-shrink-0 items-center justify-center overflow-hidden border-2 cursor-pointer hover:opacity-90 transition-opacity" style={{ borderColor: '#E9A38E' }}>
